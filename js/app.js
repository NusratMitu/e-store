...
const showProducts = products => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image mb-2" src=${product.image}>
      ...
      <p class="mb-0">${product.rating.rate} <i class="fas fa-star fa-sm"></i></p>
      <p class="text-secondary">${product.rating.count} Reviews</p>
...
// get price function
const getPrice = id => {
  const priceInString = document.getElementById(id).innerText;
  const converted = parseFloat(priceInString);  
  return converted;
}
...
const updateTotal = () => {
  const grandTotal =
    getPrice("price") + getPrice("delivery-charge") +
    getPrice("total-tax");
